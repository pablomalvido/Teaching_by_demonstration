...
def classification_calc(segmentation, process_list):
	...
	for process in process_list:
		...
		n_starts = min(max_sep*2 - 1, seq_len*2 - 1)
		for i in range(n_starts):
		    end = False
		    trans = False
		    initial_seq_calc(i-int(n_starts/2), seq_len, segm_len, max_sep)
		...
		max_prob_temp = 0
		max_seq = 99
		for seq_check in seq_analysis_process:
			prob_temp = []
			for i in range(seq_len):
				#prob of operations_segm[seq_check[i]] being process_list[process][i]
				if seq_check[i] < 0: #Outside left
					prob_temp.append(0)
				elif seq_check[i] >= segm_len: #Outside right
					prob_temp.append(0)
				else:
					if operations_segm[seq_check[i]] == process_list[process][i]:
						prob_temp.append(1)
					else:
						prob_temp.append(0.1)
...
for seq_name in sequences_list:
	...
	process_classified = classification_calc(segmentation, process_list)

	summary_calc(segmentation, seq_name)

	print(process_classified)
