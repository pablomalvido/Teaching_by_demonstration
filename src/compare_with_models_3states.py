...
for operation in operations:
  ...
  if operation == "grasp":
	...
	for model in models:
		...
		for line in f:
			line = line.replace("\n", "")
			if line == "Start:" or line == "Transition:" or line == "Map:" or line == "-" or line == "--":
				prev_line = line
				if line == "Transition:":
		...
		for line in f:
			...
			if line in model_dict[operation][model[:-4]]['map'].keys():
				...
				if first_line:
					...
					if model_dict[operation][model[:-4]]['start'][2][map_line] != 0:
						prob[operation][model[:-4]] *= model_dict[operation][model[:-4]]['start'][2][map_line]/max(model_dict[operation][model[:-4]]['start'][2])
						third_line = False
					else:
						prob[operation][model[:-4]] = 0
						break
				else:
					if model_dict[operation][model[:-4]]['trans'][prev_map_line3][prev_map_line2][prev_map_line1][map_line] != 0:
						prob[operation][model[:-4]] *= model_dict[operation][model[:-4]]['trans'][prev_map_line3][prev_map_line2][prev_map_line1][map_line]/max(model_dict[operation][model[:-4]]['trans'][prev_map_line3][prev_map_line2][prev_map_line1])
					else:
						prob[operation][model[:-4]] = 0
						break
			elif line == "":
