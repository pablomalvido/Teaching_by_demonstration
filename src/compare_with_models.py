...
path2 = os.path.join(os.path.dirname(__file__), '../files/bottle/place_top/observations/7/') #observation
model_dict = {}
prob = {}
avg_prob = {}
...
for operation in operations:
  if operation == "grasp" or operation == "grasp_top" or operation == "pour_water" or operation == "place" or operation == "place_top":
	...
	for model in models:
		...
		for line in f:
			...
			if line in model_dict[operation][model[:-4]]['map'].keys():
				...
				if first_line:
					if model_dict[operation][model[:-4]]['start'][map_line] > 0:
					...
					if model_dict[operation][model[:-4]]['trans'][prev_map_line][map_line] > 0:
						prob[operation][model[:-4]] *= model_dict[operation][model[:-4]]['trans'][prev_map_line][map_line]/max(model_dict[operation][model[:-4]]['trans'][prev_map_line])
					else:
						prob[operation][model[:-4]] = 0
						break
			elif line == "":
