...
#ROS init
rospy.init_node('compare_sequence_group_classify')
...
def prob_seq_calc(models_1, models_2, models_3, path, files_models, models_info, seq_name, seq_number):
  """
  Obtain the probabilities
  """
  #Initialization
  path_seq = path + "sequence/seq_groups/" + seq_name + "/" + seq_number + "/"
...
def classification_calc(segmentation, process_list):
	...
	cad_text = ""
	if max_process != "None":
		for process in seq_analysis:
			cad_classified_proc = []
			#print(complete_seq_analysis[process]['seq'])
			for index in complete_seq_analysis[process]['seq']:
				if index < 0 or index == 99 or index > (len(locations_segm)-1):
					cad_classified_proc.append("Unknown")
					#cad_text += "_Unknown"
				else:
					cad_classified_proc.append(locations_segm[index])
					#cad_text += "_" + str(locations_segm[index])
			cad_classified_dict[process] = cad_classified_proc

	return max_process, seq_analysis, cad_classified_dict
	


def select_max_prob(all_seqs_prob):
	#print(all_seqs_prob)
	all_probs = {}
	n_seqs = 0.0
	for process in all_seqs_prob[0]['prob']:
		all_probs[process] = 0

	for seq in all_seqs_prob:
		for process in seq['prob']:
			all_probs[process] += seq['prob'][process]
		n_seqs += 1.0

	max_process = max(all_probs, key=all_probs.get)

	if all_probs[max_process]/n_seqs <= 0.3:
		max_process = None

	cad_positions = []
	cad_positions_result = []
	cad_text = ""		
	if max_process != None:
		#Populate list with the required number of positions
		for seq in all_seqs_prob:
			if len(seq['cad']) == 0:
				continue
			for location in seq['cad'][max_process]:
				cad_positions.append({})
			break
		for seq in all_seqs_prob:
			index = 0
			if len(seq['cad']) > 0: #Can be empty if None
				for location in seq['cad'][max_process]:
					if location not in cad_positions[index]:
						cad_positions[index][location] = 1.0 * seq['prob'][max_process]
					else:
						cad_positions[index][location] += 1.0 * seq['prob'][max_process] #Ponderate (1 * is to make it more clear that the probability is just a weight)
					index += 1

		#print("AAA")
		print(cad_positions)
		for index in cad_positions:
			if len(index)>1 and "Unknown" in index:
				index.pop("Unknown")
			max_loc_index = max(index, key=index.get)
			cad_positions_result.append(max_loc_index)
			cad_text += str(max_loc_index) + ","

	return max_process, all_probs[max_process]/n_seqs, cad_positions_result, cad_text[:-1]
...
def summary_calc_groups(process_group_classified, seq_name, cad_classified):
	global process_detection
	global process_classified_correctly
	global process_and_loc_classified_correctly
	global total_location_seqs
	...
	if len(process_detection) > 0:
		if not(process_name in process_detection): #If it didn't exist yet
			process_detection[process_name] = {'amount': 0, 'correct': 0}
	else:
		process_detection[process_name] = {'amount': 0, 'correct': 0}
	process_detection[process_name]['amount'] += 1

	if process_name == process_group_classified:
		process_classified_correctly += 1
		process_detection[process_name]['correct'] += 1
		if location_included and locations_seq == cad_classified:
			process_and_loc_classified_correctly += 1
...
path_seq = path_object + "sequence/seq_groups/"
...
process_list = {}
remove_item = len(object_name) + 1
path_list = path_object[:-remove_item] + "zz_other/process_list/" + str(object_name) + ".txt"
f = open(path_list, "r")
for line in f:
	line = line.replace("\n", "")
	line = line.replace(" ", "")
	line = line.replace("_", "")
	operations_seq = line.split(",")
	process_name = ""
	for elem in operations_seq:
		process_name += ("_" + elem)
	process_list[process_name[1:]] = operations_seq
#print(process_list)
...
for seq_name in sequences_list:
	path_seq2 = path_seq + str(seq_name) + "/"
	seq_number_list = os.listdir(path_seq2)
	print(str(seq_name) + " with " + str(len(seq_number_list)) + " demos:")
	all_seqs_prob = []
	for seq_number in seq_number_list:

		times, prob_start, prob_trans = prob_seq_calc(models_1, models_2, models_3, path_object, file_names, models_info, seq_name, seq_number)
		...
		process_classified, all_process_prob, cad_classified_dict = classification_calc(segmentation, process_list)

		all_seqs_prob.append({'prob': all_process_prob, 'cad': cad_classified_dict})
		

	process_group_classified, process_prob, cad_classified, cad_text = select_max_prob(all_seqs_prob)

	#summary_calc(process_classified, seq_name, cad_classified)

	summary_calc_groups(process_group_classified, seq_name, cad_classified)

	print("Classified as: " + process_group_classified + " (" + str(process_prob*100) + "%) in: " + cad_text)
...
print("Processes classification performance decomposed:")
for process in process_detection:
	print(process + " correctly classified: " + str(process_detection[process]['correct']) + "/" + str(process_detection[process]['amount']) + ", " + str(float(process_detection[process]['correct'])/float(process_detection[process]['amount'])*100) + "%")
print("\n")
