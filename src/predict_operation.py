...
#ROS init
rospy.init_node('predict_operation')
object_name = rospy.get_param('~object', "")
operation_name = rospy.get_param('~operation', "")
demo_name = rospy.get_param('~n', "")
...
def compare_model(op, obs, model_dict):
    ...
    start_model_n = 0
    trans_model_n = 0
    path_obs = path + op + "/demos/" + str(obs)
	...
	for model in files:
		...
		trans_model_n = min(int(model_dict[3][operation_model][model_name]['n_states']), int(models_info[model[1:]]['trans']))
		#if model == "hand_orientation":
		#	trans_model_n = 1
		#else:
		#	start_model_n = 2
		#	trans_model_n = 2
		start_model_n = max(start_model_n, trans_model_n)
		#prob_model = 1
		...
		for line in f:
			...
			if line in model_dict[3][operation_model][model_name]['map'].keys():
				...
				#Second line prob
					...
					if model_name=="hand_rotation" and operation=="grasp":
						print("PROB")
						print(map_line)
						print(model_dict[3][operation_model][model_name]['start'][1])
	...
	for model in files:
		if operation=="grasp":
			#print(str(model_name) + ": " + str(prob[operation][model_name]))
			pass
		model_name = model[1:-4]
...
if op == operation_name:
	model_dict[1]['excluded'] = define_model_1(op, exclude_obs = demo_name) #The model of the operation of the observed demo is calculated again excluding that observation
	model_dict[2]['excluded'] = define_model_2(op, exclude_obs = demo_name)
	model_dict[3]['excluded'] = define_model_3(op, exclude_obs = demo_name)

result_op, results = compare_model(operation_name, demo_name, model_dict)
print("")
print("LIKELIHOOD OF THE REQUESTED DEMO WITH ALL THE " +str(object_name)+" OPERATION MODELS:")
for op in results:
	print("\t - "+str(op)+": "+str(results[op]))
print("")
print("THE OPERATION WITH THE HIGHEST LIKELIHOOD IS: " + str(result_op) + " ("+str(results[result_op])+")")
print("")
