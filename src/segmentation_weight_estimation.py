...
#ROS init
rospy.init_node('model_weight_estimation')
...
#seq_name = "3_grasp_pourwater_place" #3
...
#models_info = {"hand_xyz.txt": {"start": 2, "trans": 2, "weight": 1}, "hand_rotation.txt": {"start": 2, "trans": 2, "weight": 1}, "demonstration_complete.txt": {"start": 3, "trans": 2, "weight": 1}, "demonstration.txt": {"start": 3, "trans": 2, "weight": 1}, "hand_orientation_Z.txt": {"start": 1, "trans": 1, "weight": 1}, "z.txt": {"start": 2, "trans": 2, "weight": 1}}
#models_info_temp = {"hand_xyz.txt": {"start": 2, "trans": 2, "weight": 1}, "hand_rotation.txt": {"start": 2, "trans": 2, "weight": 1}, "demonstration_complete.txt": {"start": 3, "trans": 2, "weight": 1}, "demonstration.txt": {"start": 3, "trans": 2, "weight": 1}, "hand_orientation_Z.txt": {"start": 1, "trans": 1, "weight": 1}, "z.txt": {"start": 2, "trans": 2, "weight": 1}}
models_info = {"hand_xyz.txt": {"start": 2, "trans": 2, "weight": 1}, "hand_rotation.txt": {"start": 2, "trans": 2, "weight": 1}, "demonstration_complete.txt": {"start": 3, "trans": 2, "weight": 3}, "demonstration.txt": {"start": 3, "trans": 2, "weight": 1}, "hand_orientation_Z.txt": {"start": 1, "trans": 1, "weight": 1}, "z.txt": {"start": 2, "trans": 2, "weight": 1}, "finger_bending.txt": {"start": 2, "trans": 2, "weight": 1}, "finger_pressure.txt": {"start": 2, "trans": 2, "weight": 1}}
models_info_temp = {"hand_xyz.txt": {"start": 2, "trans": 2, "weight": 1}, "hand_rotation.txt": {"start": 2, "trans": 2, "weight": 1}, "demonstration_complete.txt": {"start": 3, "trans": 2, "weight": 3}, "demonstration.txt": {"start": 3, "trans": 2, "weight": 1}, "hand_orientation_Z.txt": {"start": 1, "trans": 1, "weight": 1}, "z.txt": {"start": 2, "trans": 2, "weight": 1}, "finger_bending.txt": {"start": 2, "trans": 2, "weight": 1}, "finger_pressure.txt": {"start": 2, "trans": 2, "weight": 1}}
...
if object_name =="bottle":
	...
	models_info_temp = {"hand_xyz.txt": {"start": 2, "trans": 2, "weight": 1}, "hand_rotation.txt": {"start": 2, "trans": 2, "weight": 1}, "demonstration_complete.txt": {"start": 3, "trans": 2, "weight": 3}, "demonstration.txt": {"start": 3, "trans": 2, "weight": 1}, "hand_orientation_Z.txt": {"start": 1, "trans": 1, "weight": 1}, "z.txt": {"start": 2, "trans": 2, "weight": 1}}
		...
		for txt_model in files_models:
			...
			for operation in operations:
			  if operation != "sequence" and operation != "shake":
				...
				#print(txt_model)
				#print(path)
				#print(prob_start[operation][txt_model[:-4]])
...
def save_models_weights(models_info):
	global object_name
	save_path = os.path.join(os.path.dirname(__file__), '../files/zz_other/models_weights/' + str(object_name) + ".txt")
	f_weights = open(save_path, "w")

	#Names operations
	models_line = ""
	for model in models_info:
		models_line = str(model) + ":" + str(models_info[model]['weight'])
		f_weights.write(models_line + "\n")
	f_weights.close()
...
model_index = 0
modifications = False
model_txt = model_txt_list[model_index]
...
while not estimated:
  ...
  for seq_name in sequences_list:
	#print(seq_name)
	times, prob_start, prob_trans = prob_seq_calc(models_1, models_2, models_3, path_object, file_names, models_info_temp, seq_name)

	times_total, prob_total_avg_start, prob_total_avg_trans = prob_avg_calc(times, prob_start, prob_trans, path_object, file_names, models_info_temp)
  ...
  performance = (3.5 * perfect_segm_perc + 2.5 * recognized_op_100_perc + 1.5 * recognized_op_50_perc + recognized_op_1_perc - 2 * wrong_op_perc)/7.5

  if performance >= best_performance:
	models_info = copy.deepcopy(models_info_temp)
	print(models_info)
	if performance > best_performance:
		modifications = True
	...
	if pos_increase: #Increase weight
		if models_info[model_txt]['weight'] < 10:
			models_info_temp[model_txt]['weight'] += 1
		else:
			pos_increase = False

	elif models_info[model_txt]['weight'] > 0: #Decrease weight
		models_info_temp[model_txt]['weight'] -= 0.5

	else: #Check next model
		pos_increase = True
		if ((model_index + 1) < len(model_txt_list)):
			model_index += 1
		else:
			if modifications:
				model_index = 0
	...
	models_info_temp = copy.deepcopy(models_info)
	if pos_increase:
		pos_increase = False
	else:
		pos_increase = True
...
save_models_weights(models_info)
...
wrong_op_perc = (float(wrong_op)/float(total_op))*100
performance = (3.5 * perfect_segm_perc + 2.5 * recognized_op_100_perc + 1.5 * recognized_op_50_perc + recognized_op_1_perc - 2 * wrong_op_perc)/7.5
#performance = (3 * perfect_segm_perc + 2 * recognized_op_100_perc + recognized_op_50_perc - 2 * wrong_op_perc)/6
#performance = (3 * perfect_segm_perc + 2.5 * recognized_op_100_perc + 1.5 * recognized_op_50_perc - 2 * wrong_op_perc)/7
