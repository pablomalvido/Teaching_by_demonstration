	...
	for model in files:
		...
		for line in f:
			...
			if line in model_dict[operation_model][model[1:-4]]['map'].keys():
				...
				if first_line:
					if model_dict[operation_model][model[1:-4]]['start'][map_line] > 0:
						prob[operation][model[1:-4]] = model_dict[operation_model][model[1:-4]]['start'][map_line]/max(model_dict[operation_model][model[1:-4]]['start'])
						first_line = False
					else:
						prob[operation][model[1:-4]] = 0
						break
				else:
					if model_dict[operation_model][model[1:-4]]['trans'][prev_map_line][map_line] > 0:
						prob[operation][model[1:-4]] *= model_dict[operation_model][model[1:-4]]['trans'][prev_map_line][map_line]/max(model_dict[operation_model][model[1:-4]]['trans'][prev_map_line])
					else:
						prob[operation][model[1:-4]] = 0
						break
			elif line == "":
